...
- **LLM 文档一致性检查**（需在“文档检查”页面操作）：自动对比文档内容与项目代码，发现描述不一致或过时的问题

**B. 可选的自动化测试/覆盖率（需要你配置命令）**

- 开关：设置环境变量 `QT_TEST_AI_ENABLE_AUTOMATION=1`
- 自动生成 QtTest 用例文件（输出到本机 `tests/generated/` 目录，避免与原有测试混淆）
...
本工具提供了专门的“文档检查”页面（导航栏点击“文档检查”），用于深度分析项目文档。

功能包括：
1. **扫描文档**：自动查找项目中的 README、使用手册、docs 目录及文件名包含“文档”的 .doc/.docx 文件
2. **LLM 一致性检查**（需配置 LLM）：
   - 自动读取文档内容（前 3 个主要文档）
   - 自动分析项目代码结构
   - 智能比对两者，找出：文档提及但代码未实现的功能、代码已实现但文档未说明的功能、API 名称不一致等问题

使用步骤：
1. 切换到“文档检查”页
2. 点击 `扫描文档`，确认列表加载正确
3. 点击 `LLM 一致性检查`，等待 AI 分析并查看右侧结果列表
